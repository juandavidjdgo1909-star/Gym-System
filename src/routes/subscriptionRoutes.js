...
// Ruta para listar todas las suscripciones.
...
// Ruta para listar suscripciones de un usuario.
...
// Ruta para consultar una suscripcion por id.
...
// Ruta para crear una suscripcion.
router.post("/", async (req, res) => {
  try {
    ...
    const isActiveMembershipError = err.message.includes("membresia activa");
    res
      .status(isActiveMembershipError ? 409 : 400)
      .json({ message: "Error al crear suscripción", error: err.message });
  }
});

// Ruta para actualizar una suscripcion.
